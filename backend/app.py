"""
# ============================================================================
# LEGACY FILE - NOT USED BY LIVE SERVER
# ============================================================================
# This is the original Flask application (v7.0).
# The live server (server.py) runs backend/main.py which is FastAPI v8.0.
# This file is kept as a reference ONLY. Do not add features here.
# All new API routes must go into backend/main.py.
# ============================================================================
NEWSCAT v7.0 - Expert Edition Flask Application (LEGACY)
...
def get_classifier():
    ...
    if _classifier is None:
        try:
            ...
            logger.info(
                f"Primary Classifier loaded: {_classifier.name} v{_classifier.version}"
            )
            ...
            try:
                ...
                logger.warning(
                    f"Fallback Classifier loaded: {_classifier.name} v{_classifier.version}"
                )
...
def cached_classification(text_hash: str, text: str):
    ...
    result = classifier.classify(text, include_confidence=True, include_all_scores=True)
...
def auth_validate():
    ...
    if token in sessions:
        ...
        return jsonify(
            {
                "status": "success",
                "data": {"username": session["username"], "valid": True},
            }
        )
...
def auth_session():
    ...
    return jsonify(
        {
            "status": "success",
            "data": {"authenticated": True, "message": "Session active"},
        }
    )
...
def processor_status():
    ...
    try:
        ...
        status["image"] = {
            ...
            "engine": img_proc.ocr_engine
            if hasattr(img_proc, "ocr_engine") and img_proc.ocr_engine != "none"
            else None,
            "installation_instructions": img_proc.get_installation_instructions()
            if not img_proc.is_available()
            else None,
        }
    except Exception as e:
        status["image"] = {
    ...
    try:
        ...
        status["audio"] = {
            ...
            "engine": audio_proc.stt_engine
            if hasattr(audio_proc, "stt_engine") and audio_proc.stt_engine != "none"
            else None,
            "ffmpeg_available": audio_proc.preprocessor.is_ffmpeg_available()
            if hasattr(audio_proc, "preprocessor")
            else None,
            "installation_instructions": audio_proc.get_installation_instructions()
            if hasattr(audio_proc, "get_installation_instructions")
            and not audio_proc.is_available()
            else None,
        }
    except Exception as e:
        status["audio"] = {
    ...
    try:
        ...
        status["video"] = {
            ...
            "dependencies": vid_proc.get_dependencies_status()
            if hasattr(vid_proc, "get_dependencies_status")
            else {},
            "installation_instructions": vid_proc.get_installation_instructions()
            if not vid_proc.is_available()
            else None,
        }
    except Exception as e:
        status["video"] = {
    ...
    return jsonify({"status": "success", "processors": status})
...
def classify():
    ...
    try:
        ...
        cached_result_str = cached_classification(text_hash, text)
        ...
        content_summary = ContentSummarizer.summarize(
            text, max_sentences=2, max_words=30
        )
        ...
        response = format_classification_result(
            ...
            subtopic=main_topic,
            main_topic_summary=classification_result.get(
...
def classify_image():
    ...
    try:
        ...
        if "image" not in request.files:
            logger.warning(
                f"No image file in request.files. Available keys: {list(request.files.keys())}"
            )
            return (
        ...
        if not result.success:
            return jsonify({"status": "error", "message": result.error_message}), 400
        ...
        # Enrich extracted_text with filename context for better classification
        # When OCR text is short/garbled, the filename (e.g. 'police_arrest.jpg') 
        # provides critical keyword context
        filename_hint = os.path.splitext(file.filename)[0].replace("_", " ").replace("-", " ")
        classification_input = f"{filename_hint} {extracted_text}".strip()
        ...
        # Use unified format_classification_result for consistent response format
        response = format_classification_result(
            ...
            category_display=classification_result.get(
                "category_display",
                classification_result["category"].replace("_", " ").title()
            ),
            content_metrics={
                "character_count": len(extracted_text),
                "extracted_text_length": len(extracted_text),
                "ocr_confidence": result.confidence or 0,
            },
        )
        
        # Add extracted text to response
        response["data"]["extracted_text"] = extracted_text[:500]
        response["data"]["ocr_confidence"] = result.confidence
        ...
        return (
            jsonify(
                {"status": "error", "message": f"Image processing failed: {str(e)}"}
            ),
            500,
        )
...
def classify_audio():
    ...
    try:
        ...
        if "audio" not in request.files:
            logger.warning(
                f"No audio file in request.files. Available keys: {list(request.files.keys())}"
            )
            return (
                ...
                jsonify(
                    {
                        ...
                        "ffmpeg_available": processor.preprocessor.is_ffmpeg_available()
                        if hasattr(processor, "preprocessor")
                        else None,
                        "installation_instructions": processor.get_installation_instructions()
                        if hasattr(processor, "get_installation_instructions")
                        else "Install: pip install openai-whisper SpeechRecognition",
                    }
                ),
                503,
            )
        ...
        if not extracted_text or len(extracted_text.strip()) < 10:
            ...
            fallback_text = (
                os.path.splitext(file.filename)[0].replace("_", " ").replace("-", " ")
            )
        ...
        # Enrich extracted_text with filename context for better classification
        # When STT transcript is short, the filename provides critical keyword context
        filename_hint = os.path.splitext(file.filename)[0].replace("_", " ").replace("-", " ")
        classification_input = f"{filename_hint} {extracted_text}".strip()
        ...
        # Use unified format for consistent response
        response = format_classification_result(
            category=classification_result["category"],
            confidence=classification_result["confidence"],
            top_predictions=top_predictions,
            ...
            content_metrics={
                "character_count": len(extracted_text),
                "transcription_confidence": result.confidence or 0,
        ...
        response["data"]["extracted_text"] = extracted_text[:500]
        response["data"]["transcription_confidence"] = result.confidence
...
def classify_video():
    ...
    try:
        ...
        if "video" not in request.files:
            logger.warning(
                f"No video file in request.files. Available keys: {list(request.files.keys())}"
            )
            return (
                ...
                jsonify(
                    {
                        ...
                        "dependencies": processor.get_dependencies_status()
                        if hasattr(processor, "get_dependencies_status")
                        else {},
                        "installation_instructions": processor.get_installation_instructions(),
                    }
                ),
        ...
        if not extracted_text or len(extracted_text.strip()) < 10:
            ...
            if len(fallback_text.strip()) >= 5:
                extracted_text = fallback_text
                logger.info(
                    f"Using filename as fallback text for video: {extracted_text}"
                )
            else:
                return (
                    jsonify(
        ...
        # Enrich extracted_text with filename context for better classification
        # When video frame OCR/audio is short, filename provides critical keyword context
        filename_hint = os.path.splitext(file.filename)[0].replace("_", " ").replace("-", " ")
        classification_input = f"{filename_hint} {extracted_text}".strip()
        ...
        response = format_classification_result(
            ...
            content_metrics={
                "character_count": len(extracted_text),
                "frames_processed": result.frames_processed or 0,
                "duration_seconds": result.duration or 0,
            },
        )
        ...
        response["data"]["extracted_text"] = extracted_text[:500]
        response["data"]["frames_processed"] = result.frames_processed
        response["data"]["duration"] = result.duration
...
def get_model_info():
    ...
    response = format_model_info(
        ...
        if isinstance(classifier.categories, list)
        else list(classifier.categories.keys()),
    ...
    news_items = [
        {
            ...
            "content": "The new model shows significant improvements in complex logic, mathematics, and coding tasks, pushing the boundaries of artificial intelligence capabilities.",
            "source": "Tech Innovations",
        },
        {
            "id": "2",
            "title": "Federal Reserve holds interest rates steady",
            "content": "Central bank officials decided to keep rates unchanged as inflation data matches expectations, signaling stability in global financial markets.",
            "source": "Finance Weekly",
        },
        {
            "id": "3",
            "title": "Real Madrid wins Champions League Final",
            "content": "A stunning late goal secured the victory for Real Madrid in a thrilling match against Manchester City, crowning them the undisputed kings of European football.",
            "source": "Global Sports",
        },
        {
            "id": "4",
            "title": "New study shows coffee may increase lifespan",
            "content": "Researchers found that drinking 2-3 cups of coffee daily is associated with a lower risk of heart disease and cardiovascular abnormalities.",
            "source": "Health Science Daily",
        },
        {
            "id": "5",
            "title": "NASA's Artemis program delayed by two years",
            "content": "Technical challenges with the lunar lander have pushed back the targeted human return to the moon, emphasizing the complexities of deep space exploration.",
            "source": "Space Exploration News",
        },
        {
            "id": "6",
            "title": "Global leaders sign new climate pact",
            "content": "At the COP summit, nations agreed to triple renewable energy capacity by 2030, marking a monumental shift toward sustainable environmental policy.",
            "source": "Eco World",
        },
        {
            "id": "7",
            "title": "Cybersecurity firm uncovers massive data breach",
            "content": "Hackers infiltrated multiple enterprise databases using a zero-day exploit, exposing the personal records of over 50 million global users.",
            "source": "Cyber Defense Network",
        },
    ]
    ...
    try:
        ...
        response = {
            ...
            "data": {
                ...
                "successful_models": [
                    m for m, r in result.model_results.items() if r.success
                ],
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting NEWSCAT API server on {Config.HOST}:{Config.PORT} ({'development' if Config.DEBUG else 'production'} mode)"
    )
    # Run the Flask app
    app.run(
        host=Config.HOST, port=Config.PORT, debug=Config.DEBUG, threaded=Config.THREADED
    )
